...
" is not near to " << #b << " = " << b << " within the relative tolerance " << tolerance << std::endl; CHECK(std::abs(b) <= std::numeric_limits<double>::epsilon());} else {DOCTEST_CHECK_NEAR(a,b,tolerance);};
